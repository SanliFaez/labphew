...
class ScanIVWindow(ScanWindowBase):
    ...
    def set_UI(self):
        ...
        self.R_spinbox = QSpinBox(suffix='R', minimum=1, maximum=100e6, valueChanged=self.changeR)
        ...
        layout_scan_form.addRow(self.scan_stop_label, self.scan_stop_spinbox)
        layout_scan_form.addRow(self.scan_step_label, self.scan_step_spinbox)
        layout_scan_form.addRow(QLabel('resistor'), self.R_spinbox)
    ...
    def changeR(self, new_value):
        self.operator.properties[self.scan_name]['R'] = new_value
    ...
    def update_scan(self):
        ...
        if self.operator._new_scan_data:
            self.operator._new_scan_data = False
            self.curve1.setData(self.operator.V_f, self.operator.I_f)
            self.plot1.setXRange(0, max(self.operator.V_f))
        if self.scan_thread.isFinished():
            self.logger.debug('Scan thread is finished')
            self.scan_timer.stop()
            self.reset_fields()
