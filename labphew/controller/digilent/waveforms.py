...
class DfwController(dwf.Dwf):
    def __init__(self, device_number=0, config=0):
        ...
        self._read_simple_analog_return_std = False


    def preset_simple_analog(self, n=84, freq=10000, range=50.0, mult=(1.0, 1.0), return_std=False):
        """
        Apply settings for read_analog()
        Please note that there may be a significant overhead (delay) which seems to be larger for lower frequencies
        and oddly seems to be larger for collecting a small number of points.
        ...
        daq.ai.frequencySet(freq)
        daq.ai.channelRangeSet(-1, range)
        daq.ai.configure(False, True)  # apply config
        time.sleep(0.1)

    def read_simple_analog(self):
        """Simplified method to read the two analog IN channels"""
        daq.ai.configure(False, True)  # start acquisition
        ...
        if self._read_simple_analog_return_std:
...
if __name__ == '__main__':
    ...
    # example showing read_simple_analog()

    ch_out = 0
    daq.ao.configure(ch_out, 0)
    daq.ao.reset()
    daq.ao.nodeFunctionSet(ch_out, daq.ao.NODE.CARRIER, daq.ao.FUNC.DC)
    daq.ao.nodeOffsetSet(ch_out, daq.ao.NODE.CARRIER, -0.5)
    print("\nStarting output")
    daq.ao.configure(ch_out, True)
    time.sleep(0.1)  # allow output to stabilize

    daq.preset_simple_analog()
    time.sleep(1)
    print(daq.read_simple_analog())
